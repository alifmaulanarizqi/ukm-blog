          ...
          <table id="dtBasicExample" class="table">
            <thead>
              <tr>
                ...
                <th scope="col">Alasan Masuk</th>
                <th scope="col">Action</th>
              </tr>
            </thead>
            <tbody>
              @php($i = 1)
              @foreach($user_pendaftars as $row)
                  <tr>
                    <td scope="row">{{ $i++ }}</td>
                    <td>{{ $row->name }}</td>
                    <td>{{ $row->email }}</td>
                    <td>{{ $row->reason_joining }}</td>
                    <td>
                      <a href="{{ route('show.userpendaftar', $row->id) }}" class="btn btn-info btn-sm">Show</a>
                      <a href="{{ route('aprrove.user', $row->id) }}" class="btn btn-success btn-sm">Approve</a>
